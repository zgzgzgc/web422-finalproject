...
const Wishlist = () => {
    ...
    try {
      ...
      const gamesWithDetails = await Promise.all(
        games.map(async (appid) => {
          ...
          return { ...gameData, coverImage, appid }; // Ensure appid is included
        })
      );

      console.log("Fetched Wishlist Games:", gamesWithDetails); // Debugging line
      setWishlistGames(gamesWithDetails);
      setTotalPages(Math.ceil(gamesWithDetails.length / ITEMS_PER_PAGE)); // Calculate total pages
    } catch (err) {
  ...
  const displayedGames = wishlistGames.slice(
    ...
    <div style={styles.container}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '20px' }}>
        ...
        <button
          ...
          onClick={() => router.push('/viewhistory')}  // Redirects to viewhistory page
        >
          Viewed History
        </button>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center' }}>
        {displayedGames.length === 0 && !loading && <p>No games in your wishlist</p>}
